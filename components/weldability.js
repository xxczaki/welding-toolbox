...
const Weldability = () => {
	...
	const [ceAws, setCeAws] = useState('0');
	...
	return (
		<Formik
			...
			onSubmit={async values => {
				...
				setCeq(equationCeq);
				setCet(equationCet);
				setCeAws(equationCeAws);
				setPcm(equationPcm);
				setPren(equationPren);
			}}
				...
				<ScrollView style={styles.container} scrollEnabled={false} keyboardShouldPersistTaps="handled">
					...
					<Grid>
						...
						<Col style={styles.column3}>
							<RkText>CEQ: {ceq}</RkText>
							<RkText>CET: {cet}</RkText>
							<RkText>CE (AWS): {ceAws}</RkText>
							<RkText>PCM: {pcm}</RkText>
							<RkText>PREN: {pren}</RkText>
						</Col>
					</Grid>
